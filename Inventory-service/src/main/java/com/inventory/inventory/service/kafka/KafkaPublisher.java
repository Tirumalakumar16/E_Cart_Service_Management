...
public class KafkaPublisher {
    ...
    public void sendMessage(String message) {
        CompletableFuture<SendResult<String, Object>> send = kafkaTemplate.send("inventory-service", message);

        send.whenComplete((result,ex) ->{
            if(ex == null) {
                log.info("From Inventory-service : " + message+" with partition : "+result.getRecordMetadata().partition()+" " +
